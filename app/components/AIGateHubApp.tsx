...
function usageRecordToSupabaseRow(record: UsageRecord) {
  return {
    user_id: demoUserId,
    type: record.type,
    model: record.model,
    prompt_summary: record.inputSummary,
    cost_cny: record.cost,
    status: record.status,
    response_time_ms: Number.parseInt(record.responseTime, 10),
  };
}

function supabaseRowToUsageRecord(row: UsageLogRow): UsageRecord {
  return {
    id: row.id,
    createdAt: row.created_at,
    time: new Date(row.created_at).toLocaleString("zh-CN", { hour12: false }),
    type: row.type,
    model: row.model,
    inputSummary: row.prompt_summary,
    cost: row.cost_cny,
    status: row.status,
    responseTime: `${row.response_time_ms}ms`,
  };
}
...
export default function AIGateHubApp({ view, scrollTarget }: { view: AppView; scrollTarget?: string }) {
  ...
  const addUsageRecord = async (record: UsageRecord) => {
    ...
    const { data, error } = await supabase
      .from("usage_logs")
      .insert(usageRecordToSupabaseRow(record))
      .select()
      .single<UsageLogRow>();

    if (error || !data) {
      console.error("Failed to write Supabase usage log", error);
      return;
    }

    setUsageRecords((current) => [
      supabaseRowToUsageRecord(data),
  ...
  useEffect(() => {
    if (supabase) {
      supabase
        ...
        .then(({ data, error }) => {
          if (error) {
            console.error("Failed to load Supabase usage logs", error);
            setUsageRecords([]);
          } else {
            setUsageRecords((data ?? []).map((row) => supabaseRowToUsageRecord(row as UsageLogRow)));
          }
    ...
    try {
      ...
      setUsageRecords(savedRecords ? JSON.parse(savedRecords) : []);
    } catch {
      setUsageRecords([]);
    } finally {
      setHasLoadedUsage(true);
  ...
  useEffect(() => {
    if (!hasLoadedUsage || isSupabaseConfigured) return;
    window.localStorage.setItem(usageStorageKey, JSON.stringify(usageRecords));
  }, [hasLoadedUsage, usageRecords]);
  ...
  if (currentView === "dashboard") {
    return (
      <DashboardPage
        ...
        onOpenImage2={() => router.push("/image2")}
  ...
  if (currentView === "chat") {
    return <ChatTestPage onBack={() => router.push("/dashboard")} onUsageRecord={addUsageRecord} />;
  }

  if (currentView === "image2") {
    return <Image2TestPage onBack={() => router.push("/dashboard")} onUsageRecord={addUsageRecord} />;
  }
    ...
    <div className="min-h-screen bg-background text-foreground overflow-x-hidden" style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 border-b border-[rgba(30,110,255,0.12)] backdrop-blur-lg" style={{ background: "rgba(5,10,20,0.85)" }}>
        <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-2 cursor-pointer" onClick={() => scrollTarget ? router.push("/") : scrollTo("hero")}>
        ...
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between gap-5">
          ...
          <div className="flex gap-5 text-xs text-muted-foreground">
            {[["隐私政策", ""], ["服务条款", ""], ["联系我们", "contact"]].map(([label, id]) => (
              <button
                key={label}
                onClick={() => id && router.push("/contact")}
